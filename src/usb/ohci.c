 ...
 * lands in low RAM -- on one title inside the title's own .data -- so the
...
static uint32_t ohci_do_td(OhciController *hc, uint32_t ed0, uint32_t td)
{
     ...
     * and follows its garbage link: a title asks for 80 bytes of
 ...
 * same TDs twice: a title faulted on a freed TD's free-list link read
