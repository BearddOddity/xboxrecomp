 ...
 * within minutes (some titles do). As a 32-bit int it wrapped negative,
...
static void bridge_NtFreeVirtualMemory(void)
{
    ...
    if (base_ptr && size_ptr) {
         ...
         * allocated this way came back, and one title's 2 MB pool segments
...
static HANDLE bridge_resolve_handle(uint32_t token)
{
     ...
     * which Win32 rejects: one title's CRT duplicates NtCurrentThread()
